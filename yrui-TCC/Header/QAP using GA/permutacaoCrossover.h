...
vector<Caminho> crossover(vector<Caminho> caminhosCrossover, vector<Caminho> caminhos, int individuoCross1, int individuoCross2) {
	...
	//Separa os indivíduos, juntando as primeiras e as segundas partes de cada parte
	for (i = 0; i < corte; i++) {
	...
	int k1 = 0;
	int p = 0;
	//Verifica os vertices que se repetem no indíviduo e substitui pelos vertices que não se repetem de forma ordenad, 0,1,2,3...
	for (i = 0; i < quantidadeVertices; i++) {
		if (i < corte) {
	...
	//adiciona os indivíduos na população
